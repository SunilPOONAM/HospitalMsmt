...
{
    public class PayTabAPI:IPayTabAPI
    {
        ...
        public PayTabAPI(IConfiguration configuration)
        {
            _configuration = configuration;
            ApiURL= _configuration.GetSection("AppSettings")["PaymentGatewayAPRURL"];
            siteURL = _configuration.GetSection("AppSettings")["SiteUrl"];
            SecretKey = _configuration.GetSection("AppSettings")["PayTabSecretKey"];
            MerchantEmail = _configuration.GetSection("AppSettings")["UserEmail"];
            MerchantPassword = _configuration.GetSection("AppSettings")["UserPassword"];
        }
        ...
        public string MakeWebSiteCallFromHandler(string formContent)
        {
            ...
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(siteURL + "/" + "IPNListener.aspx");
            request.Method = "POST";
        ...
        public string CreatePayPage(PayPageRequest objPayPageRequest)
        {
            string param = "merchant_email=" + MerchantEmail
                                ...
                                + "&return_url=" + siteURL + objPayPageRequest.ReturnUrl
            ...
            if (objPayPageRequest.is_recurrence_payments)
            {
                param += "&is_recurrence_payments=" + objPayPageRequest.is_recurrence_payments;
                param += "&recurrence_start_date=" + DateTime.Now
                + "&recurrence_frequency="+objPayPageRequest.RecurrenceFrequency
                + "&recurrence_billing_cycle="+objPayPageRequest.RecurrenceBillingCycle;
            }
            return param;
        }

        public string MakeRefund(RefundRequest objRefundRequest)
        {
        ...
        public string ReturnTransactionReport(ReportRequest objReportRequest)
        {
        ...
        public string PayTabsIPN(ReportRequest objReportRequest)
        {
            return "merchant_email=" + objReportRequest.MerchantEmail
                    + "&secret_key=" + SecretKey;
        }

        public string ValidateSecretKey(VerifySecretKeyRequest objRequest)
        {
            return "merchant_email=" + objRequest.MerchantEmail
                   + "&secret_key=" + SecretKey;
        }

        public VerifyPaymentResponse VerifyPayment(VerifyPaymentRequest objPaymentRequest)
        {
            ...
            return JsonConvert.DeserializeObject<VerifyPaymentResponse>(response);
        }
